            ...
            if amount_to_send > 0:
                ...
                try:
                    if self.channel_db:
                        ...
                        route, self.trampoline2 = await self.create_trampoline_route(lnaddr, self.trampoline_fee_level, self.trampoline2_list)
    ...
    def handle_error_code_from_failed_htlc(self, htlc_log):
        ...
        if self.channel_db is None:
            if code == OnionFailureCode.TRAMPOLINE_FEE_INSUFFICIENT:
                ...
                return True
            elif self.trampoline2:
                self.logger.info(f"blacklisting trampoline2 {self.trampoline2.hex()}")
                self.trampoline2_list.remove(self.trampoline2)
                return True
            else:
                return False
